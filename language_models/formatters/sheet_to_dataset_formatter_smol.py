...
output_file = os.path.join(BASE_DIR, "../datasets/finetuning_ready_dataset_1_smol.jsonl", )
...
for json_file in json_files:
    ...
    for entry in data:
        ...
        # Crear el input
        input_data = f"<|system|> Tarea: {tarea}\n\nNivel de emoción: {nivel_emocion}\n\nDatos del personaje:\n\tNombre: {personaje}\n\tDescripción del personaje: {descripcion_personaje}\n\nContexto:\n"
        ...
        prompt_usuario_modificado = prompt_usuario.replace("[user]", "<|user|>")
        respuesta_modificada = f"<|assistant|> {respuesta}"

        text = f"{input_data}{prompt_usuario_modificado}\n{respuesta_modificada}"
